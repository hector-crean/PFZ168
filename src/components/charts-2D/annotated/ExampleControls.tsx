...
import { PickD3Scale, scaleTime, scaleLinear } from "@/lib/visx-scale";
import { extent } from "d3-array";
import { Annotation, EditableAnnotation } from "@/lib/visx-annotation";
...
export default function ExampleControls({
	...
	const [title, setTitle] = useState("Refactory Relapse");
	const [subtitle, setSubtitle] = useState(
		compact
			? ""
			: "Patients with relapsed/refractory multiple myeloma (RRMM) often have a sustained response to front-line therapy for several years",
	);
	const [connectorType, setConnectorType] =
		useState<ProvidedProps["connectorType"]>("elbow");
	const [subjectType, setSubjectType] =
		useState<ProvidedProps["subjectType"]>("circle");
