...
interface TimeInterval<T> extends RangeTrait<string> {
    tag: 'time-inteveral',
    kind: T;
    description: string;
}
...
const extractDatesFromTimeline = <I, E, Is extends Array<I>, Es extends Array<E>>({ events, intervals }: Timeline<I, E, Is, Es>) => {
    const merged = [...events.map(event => event.date), ...intervals.map(interval => interval.start), ...intervals.map(interval => interval.end)]
    return merged
}

const extractDatesFromTimelines = <I, E, Is extends Array<I>, Es extends Array<E>>(timelines: Array<Timeline<I, E, Is, Es>>) => {
