...
import { LinearGradient } from '@/lib/visx-gradient';
import { Group } from '@/lib/visx-group';
import { getRandomNormal, getSeededRandom } from '@/lib/visx-mock-data';
import genStats, { Stats } from '@/lib/visx-mock-data/generators/genStats';
...
import { BoxPlot, ViolinPlot } from '@/lib/visx-stats';
import { Tooltip, defaultStyles as defaultTooltipStyles, withTooltip } from '@/lib/visx-tooltip';
import { WithTooltipProvidedProps } from '@/lib/visx-tooltip/enhancers/withTooltip';
...
const RaincloudPlot = withTooltip<StatsPlotProps, TooltipData>(
	({
		width,
		height,
		tooltipOpen,
		...
		return width < 10 ? null : (
			<div style={{ position: 'relative' }}>
				<svg width={width} height={height}>
					<LinearGradient id="statsplot" to="hsl(205, 100%, 30%)" from="hsl(205, 100%, 39%)" />
					<rect x={0} y={0} width={width} height={height} fill="url(#statsplot)" />
					<PatternLines
						id="hViolinLines"
						height={3}
						width={3}
...
const ResponsiveRaincloudPlot = () => (<ParentSize>{({ width, height }) => <RaincloudPlot width={width} height={height} />}</ParentSize>)


export { ResponsiveRaincloudPlot };
