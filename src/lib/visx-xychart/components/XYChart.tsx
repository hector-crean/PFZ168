...
import ParentSize from '@/lib/visx-responsive/components/ParentSize';
import { ResizeObserverPolyfill } from '@/lib/visx-responsive/types';
...
>(props: XYChartProps<XScaleConfig, YScaleConfig, Datum>) {
  ...
  if (!setDimensions) {
    if (!xScale || !yScale) {
      console.warn(
        '[@/lib/visx-xychart] XYChart: When no DataProvider is available in context, you must pass xScale & yScale config to XYChart.',
      );
      return null;
    }
    return (
